class CoffeePost < ApplicationRecord
  ...
  enum grind_size: { unspecified_grind_size: 0, coarse: 1, medium_coarse: 2, medium: 3, medium_fine: 4, fine: 5, turkish: 6, other: 7 }
  ...
  def get_coffee_image(width, height)
    unless coffee_image.attached?
      ...
      coffee_image.attach(io: File.open(file_path), filename: 'default-image.jpg', content_type: 'image/jpeg')
    end
